...
# this is version 0.6 of module " Strong Coupling (SC) correction ".

# this module is created from version 0.5 of Module_SC_Beta.
# new stuffs are external magentic field coupling related i.e., g_H, g_z, A_2 gaps, B_2 gaps,
# as well as their bulk energies.
...
# For example, version 0.6 gives out \alpha and \beta_{i} as 
#'''
#   pnoa * alpha_td * N(0),
...
# pnoa(b), N(0), Tc and beta_td will be offered separately when they are called with " module.name " form i.e., SSC.beta_td etc.
...
m = 1;s = 1; J = 1; Kelvin = 1; kg = 1; bar = 1; Gauss=1
...
#####################################################################
####         set up the gyromagnetic ratio for helion            ####
#####################################################################

# helion/3He nucleus gyromagnetic ratio
# this data comes from NIST "shielded helion gyromagentic ratio "
# there are two possible option. One is -3243.409942*Hz*Gauss**(-1),
# while the other one is -20380.16785*Hz*Gauss**(-1)

# default value of gamma
gamma = -3243.409942*Hz*Gauss**(-1)

def switch_gamma_to_non_shielded():
   '''Switch the gamma from default "shielded" value to
   
   the value calculated from helion magnetic momentum. The
   
   data comes from NIST as well.
   '''
   globals()["gamma"] = -20380.16785*Hz*Gauss**(-1)
...
def set_q_order(order):
   ...
   parameter order is int type, one of 0,2,3,4,5,6.

   When "0" is set, SCC formular reduce to RWS2019. 
   '''
...
# functon for setting up the fudge correction
def fudgeq(p):
   if order_q == 0:
      return  0
   elif order_q != 0:
      return expco.q(p, order_q)
...
# dimensionless g_H i.e., \tilde{g_H}
def gH_td(p):
   '''This function returns a list of dimensional gH for H^2 coupling term.
   The dimension of the first element is J^2*Gauss^-2, while the dimension of 
   the end element is Gauss^2.
   '''
   BetaObject.F0a_function(SC.P,SC.F0A,p)
   # print(" F0a is ",BetaObject.F0a)
   pnoa, pnob = pno()
   mu=(gamma*hbar)/2

   returnList = [(5.0*pnob*4.0*mu*mu)/((1+BetaObject.F0a)**2),(5.0*pnob*4.0*mu*mu)/((kb*kb*Tcp(p)*Tcp(p))*((1+BetaObject.F0a)**2))]
   
   return returnList

# dimensionless g_z i.e.,
def gz_td(p):
   '''This parameter is determined by the critical tempreture spliting 
   of A1 phase from that of zero field A phase. The T_c^A1-Tc is propotion
   to the field. lambda^A1 is the propotional coeffcient. The pre-coeff 51
   is set as constant even though it isn't.

   This gz_td is dimensional, it's unit is Gauss^-1

   see more on PRB 68. 224502
   '''
   mu=(gamma*hbar)/2
   lambdaA1 = 51.0*abs(mu/kb)*(kb*Tcp(p)*(Ef(p)**(-1)))

   return (lambdaA1/Tcp(p))/3
...
def betaPlanar_td(p, T):
   return (2.*beta1_td(p, T) + 2.*beta2_td(p, T)
...
def fA2(p, T, h):
   ...
   betaA = pno_tuple[1]*betaA_td(p, T)*N0(p)*((kb*Tcp(p))**(-2))
  
   f = (-(alpha*alpha)/betaA + (gz*gz*h*h)/beta5)/4.

   return f

def fPlanar(p, T, h):
   '''Bulk equlibrium free energy of uniform planar phase. Unit is J*m^-3
   ...
   betaPlanar = pno_tuple[1]*betaPlanar_td(p, T)*N0(p)*((kb*Tcp(p))**(-2))
   ...
   f = -(alpha*alpha)/betaPlanar + (gz*gz*h*h)/(2.*beta1+beta3-beta4+beta5)
...
def fB2_noPHA(p, T, h):
   ...
   betaB = pno_tuple[1]*betaB_td(p, T)*N0(p)*((kb*Tcp(p))**(-2))
   beta345 = pno_tuple[1]*(beta3_td(p, T)+beta4_td(p, T)+beta5_td(p, T))*N0(p)*((kb*Tcp(p))**(-2))
   beta12 = pno_tuple[1]*(beta1_td(p, T)+beta2_td(p, T))*N0(p)*((kb*Tcp(p))**(-2))
   beta1 = pno_tuple[1]*beta1_td(p, T)*N0(p)*((kb*Tcp(p))**(-2))
   beta2 = pno_tuple[1]*beta2_td(p, T)*N0(p)*((kb*Tcp(p))**(-2))

   f1 = -((gH*h*h+3.*alpha)**2)/(36.*betaB)
   f2 = -(gH*gH*h*h*h*h)/(6.*beta345)
...
def GapA2(p, T, h):
   ...
   deltauu2 = -alpha/(2.*betaA) - (gz*h)/(2.*beta5)
   deltadd2 = -alpha/(2.*betaA) + (gz*h)/(2.*beta5)

   return np.array([deltauu2, deltadd2])

def GapPlanar(p, T, h): 
   '''Gaps of magentic planar phase under h field. 
   This function return list of squre of gaps (in SI, J^2) or 
   gaps (in SI unit, J) of planar phase
   ...
   deltauu2 = -(3.*alpha)/(2.*(betaPlanar)) - (3.*gz*h)/(2.*(2.*beta1+beta3-beta4+beta5))
   deltadd2 = -(3.*alpha)/(2.*(betaPlanar)) + (3.*gz*h)/(2.*(2.*beta1+beta3-beta4+beta5))

   return np.array([deltauu2, deltadd2])


def GapB2_noPHA(p, T, h): 
   '''Gaps of B2 phase under h field. PHA means Particle-Hole-Asymmetry.
   This function return list of squre of gaps (in SI, J^2) and 
   gaps (in SI unit, J) of B2 without PHA. Then the returned \Delta_uu is equal to \Delta_dd.

   g_z term is set to zero in this case, and this is the only case B2 gaps
   could be analytically solved by Mathematica (10.1).
   '''
   gHList = gH_td(p)
   gH = gHList[1]*N0(p)
   ...
   deltaud2 = -(gH*h*h+3.*alpha)/(6.*betaB) - (gH*h*h)/beta345
   deltauudd2 = -(3.*(-gH*h*h*beta12+alpha*beta345))/(6.*beta345*betaB)

   return np.array([deltauudd2, deltauudd2, deltaud2])
   # return deltauudd2, deltauudd2, deltaud2, np.sqrt(deltauudd2), np.sqrt(deltauudd2), np.sqrt(deltaud2)
